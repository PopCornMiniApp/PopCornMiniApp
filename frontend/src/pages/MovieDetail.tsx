...
export default function MovieDetail({ id, navigate, goBack }: Props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d" }}>
      {/* Back button — fixed, just below the phone safe area (not Telegram header) */}
      {!playing && (
      ...
      <div style={{ padding: "0 16px 80px" }}>
        <div style={{ display: "flex", gap: 14, marginTop: playing ? 16 : -46, position: "relative" }}>
          ...
          <div style={{ flex: 1, paddingTop: playing ? 0 : 48 }}>
            <h1 style={{ fontSize: 19, fontWeight: 800, lineHeight: 1.3, marginBottom: 4 }}>{title}</h1>
            {movie.title !== movie.title_ar && movie.title_ar && (
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 6 }}>{movie.title}</p>
            )}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              ...
              {runtime && <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", display: "flex", alignItems: "center", gap: 3 }}><Clock size={10} />{runtime}</span>}
            </div>
          </div>
        </div>
        ...
        {movie.has_file && !playing && (
          <button onClick={() => setPlaying(true)} style={{
            ...
            color: "#000", padding: "13px 24px", borderRadius: 16,
        ...
        {!movie.has_file && (
          <div style={{ width: "100%", marginTop: 18, background: "rgba(255,255,255,0.04)", padding: "13px 24px",
            borderRadius: 16, textAlign: "center", color: "rgba(255,255,255,0.35)", fontSize: 13,
            border: "1px dashed rgba(255,255,255,0.1)" }}>⏳ قريباً — الفيلم غير متاح بعد</div>
        )}
          ...
          <div style={{ marginTop: 18, background: "rgba(255,255,255,0.03)", borderRadius: 14, padding: "14px 16px",
            ...
            {movie.director && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 600 }}>{movie.director}</span>
              </div>
            )}
            {movie.release_date && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>تاريخ الإصدار</span>
                <span style={{ fontSize: 13 }}>{movie.release_date}</span>
              </div>
            )}
            {movie.runtime > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>المدة</span>
                <span style={{ fontSize: 13 }}>{fmtRuntime(movie.runtime)}</span>
              </div>
            )}
            {movie.vote_count > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>عدد التقييمات</span>
                <span style={{ fontSize: 13 }}>{movie.vote_count.toLocaleString()}</span>
              </div>
            )}
          </div>
        )}
