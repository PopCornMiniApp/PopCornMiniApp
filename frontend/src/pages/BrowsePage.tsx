...
export default function BrowsePage({ type: initType = "movies", genre: initGenre, navigate }: Props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            style={{
              ...
              background: showFilters ? "rgba(245,158,11,0.18)" : "rgba(255,255,255,0.07)",
              border: showFilters ? "1px solid rgba(245,158,11,0.45)" : "1px solid rgba(255,255,255,0.1)",
              color: showFilters ? "#fde68a" : "rgba(255,255,255,0.6)", fontSize: 12, fontWeight: 600,
            }}
          >
      ...
      {showFilters && (
        <div style={{
          background: "rgba(20,18,28,0.98)", padding: "16px",
          ...
          `}</style>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.38)", marginBottom: 8, fontWeight: 600, letterSpacing: 0.5 }}>الترتيب</p>
          <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
            {[{ v: "newest", l: "الأحدث" }, { v: "rating", l: "الأعلى تقييماً" }].map(({ v, l }) => (
              <button key={v} onClick={() => setSort(v as any)} style={{
                padding: "6px 16px", borderRadius: 20, fontSize: 12, fontWeight: 700,
                background: sort === v ? "#f59e0b" : "rgba(255,255,255,0.07)",
                color: sort === v ? "#000" : "rgba(255,255,255,0.5)",
                border: sort === v ? "1px solid #f59e0b" : "1px solid rgba(255,255,255,0.1)",
          ...
          {genres.length > 0 && (
            <>
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.38)", marginBottom: 8, fontWeight: 600, letterSpacing: 0.5 }}>التصنيف</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                <button onClick={() => setActiveGenre(null)} style={{
                  padding: "5px 14px", borderRadius: 20, fontSize: 11, fontWeight: 700,
                  background: !activeGenre ? "#f59e0b" : "rgba(255,255,255,0.07)",
                  color: !activeGenre ? "#000" : "rgba(255,255,255,0.5)",
                ...
                {genres.map(g => (
                  <button key={g} onClick={() => setActiveGenre(g === activeGenre ? null : g)} style={{
                    ...
                    color: activeGenre === g ? "#000" : "rgba(255,255,255,0.5)",
      ...
      {activeGenre && (
        <div style={{ padding: "8px 16px 0", display: "flex", gap: 6 }}>
          <div style={{
            ...
            {activeGenre}
            <button onClick={() => setActiveGenre(null)} style={{ color: "#fde68a", display: "flex" }}>
              <X size={12} />
            </button>
          </div>
        </div>
      ...
      {/* Grid — 3 columns, cards fill their column width */}
      <div style={{ padding: "12px 16px 90px" }}>
        {loading && items.length === 0 ? (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "column", padding: "80px 0", gap: 14 }}>
            <div style={{ width: 34, height: 34, border: "3px solid rgba(255,255,255,0.08)", borderTopColor: "#f59e0b", borderRadius: "50%", animation: "spin 0.75s linear infinite" }} />
          ...
          <>
            ...
            {hasMore && !loading && (
              <button
                ...
                style={{
                  ...
                  borderRadius: 13, color: "#fde68a", fontSize: 13, fontWeight: 700,
            ...
            {loading && items.length > 0 && (
              <div style={{ display: "flex", justifyContent: "center", padding: "24px 0" }}>
                <div style={{ width: 26, height: 26, border: "2.5px solid rgba(255,255,255,0.08)", borderTopColor: "#f59e0b", borderRadius: "50%", animation: "spin 0.75s linear infinite" }} />
              </div>
            )}
          </>
        )}
