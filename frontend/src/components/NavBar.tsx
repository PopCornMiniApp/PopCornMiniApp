...
interface Props { current: string; navigate: (r: any) => void; }
  ...
  return (
    <div style={{
      ...
      {items.map(({ key, icon: Icon, label, action }) => {
        const active = current === key || (key === "movies" && current === "browse");
        return (
          <button key={key} onClick={action} style={{
            display:"flex",flexDirection:"column",alignItems:"center",gap:3,
            padding:"4px 16px",color:active?"#8b5cf6":"rgba(255,255,255,0.38)",transition:"color 0.2s",
          }}>
