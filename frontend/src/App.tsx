...
type Route={page:"home"}|{page:"movie";id:string}|{page:"series";id:string}|{page:"search"}|{page:"browse";type?:"movies"|"series";genre?:string};

export default function App() {
  const [route,setRoute]=useState<Route>({page:"home"});
  const [history,setHistory]=useState<Route[]>([]);

  const navigate=(r:Route)=>{setHistory(h=>[...h,route]);setRoute(r);window.scrollTo(0,0);};
  const goBack=()=>{
    if(!history.length){setRoute({page:"home"});return;}
    const prev=history[history.length-1];
    setHistory(h=>h.slice(0,-1));setRoute(prev);window.scrollTo(0,0);
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0d0d0d",paddingBottom:isDetail?0:70}}>
      ...
      {route.page==="browse"&&<BrowsePage type={(route as any).type} genre={(route as any).genre} navigate={navigate} goBack={goBack}/>}
      {!isDetail&&<NavBar current={route.page} navigate={navigate}/>}
    </div>
  );
}
