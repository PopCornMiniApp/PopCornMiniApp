...
2. Distributes content across 9 mirror groups (1 main + 8 mirrors)
3. Ignores specific topics (e.g., "General" for archiving)
4. Implements fast incremental sync (every 1 minute)
5. Ensures redundancy (each file in 3 groups via mirror_manager)
6. Handles conflict resolution
7. Uses user bot for full API access
8. Integrates with mirror_manager for automatic mirroring
"""
...
from app.config import MIRROR_REDUNDANCY
...
class MultiGroupSyncManager:
    ...
    def __init__(self):
        ...
        self.group_content: Dict[int, Set[str]] = defaultdict(
            set)  # group_id -> set of file_ids
    ...
    async def _scan_forum_topics(
        ...
        try:
            # Get forum topics
            # type: ignore
            async for topic in self._user_client.get_forum_topics(group_id):
                topic_id = topic.id
    ...
    async def _extract_content_item(
        ...
        try:
            ...
            logger.error(
                f"Error extracting content from message {message.id}: {e}")
            return None

    async def sync_to_mirrors(
        self,
        content_items: List[ContentItem],
        target_redundancy: Optional[int] = None
    ) -> Dict:
        """
        Sync content items to mirror groups using mirror_manager

        Args:
            content_items: List of content items to sync
            target_redundancy: Number of mirror groups each item should be in (default: from config)
        ...
        # Use config redundancy if not specified
        if target_redundancy is None:
            target_redundancy = MIRROR_REDUNDANCY
        ...
        async with self.sync_lock:
            ...
            if not available_mirrors:
                ...
                return {"synced": 0, "failed": 0, "mirrored": 0, "duration": 0}

            logger.info(
                f"🔄 Starting sync: {len(content_items)} items to "
                f"{len(available_mirrors)} mirrors (redundancy={target_redundancy})")

            # Import mirror_manager here to avoid circular imports
            try:
                from app.mirror_manager import mirror_manager
                mirror_enabled = mirror_manager._initialized
            except Exception as e:
                logger.warning(f"Mirror manager not available: {e}")
            ...
            for i in range(0, len(content_items), batch_size):
                ...
                tasks = [
                    self._sync_item_to_mirrors(
                        item, available_mirrors, target_redundancy, mirror_enabled
                    )
                    for item in batch
                ]
            ...
            logger.info(
                f"✅ Sync complete: synced={synced_count}, mirrored={mirrored_count}, "
                f"failed={failed_count}, duration={duration:.2f}s")
    ...
    async def _sync_item_to_mirrors(
        ...
        try:
            ...
            if not target_mirrors:
                logger.warning(
                    f"No target mirrors available for item {item.file_id}")
                return False
            ...
            try:
                ...
                msg = original_msg[0] if isinstance(
                    original_msg, list) else original_msg

                if not msg or not hasattr(msg, 'media') or not msg.media:
                    logger.warning(
                        f"Could not get original message {item.message_id}")
                    return False
            ...
            for mirror_name, mirror_group in target_mirrors:
                try:
                    ...
                    if forwarded:
                        ...
                        self.group_content[mirror_group.group_id].add(
                            item.file_id)
